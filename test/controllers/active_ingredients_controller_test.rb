...
class ActiveIngredientsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should redirect show if not authenticated" do
    get active_ingredient_path(@active_ingredient)
  ...
  test "should create active ingredient with valid params" do
    ...
    assert_difference "ActiveIngredient.count" do
      post active_ingredients_path, params: { active_ingredient: { name: "Methylphenidate :3", half_life: 3.5 } }
    end
  ...
  test "should not create active ingredient with invalid params" do
    sign_in_as(@admin)
    assert_no_difference "ActiveIngredient.count" do
      post active_ingredients_path, params: { active_ingredient: { name: "", half_life: nil } }
    end
  ...
  test "should redirect edit if not authenticated" do
    get edit_active_ingredient_path(@active_ingredient)
    assert_redirected_to new_session_path
  end
  ...
  test "should redirect update if not authenticated" do
    patch active_ingredient_path(@active_ingredient), params: { active_ingredient: { name: "Updated" } }
    assert_redirected_to new_session_path
  end
  ...
  test "should update active ingredient if admin" do
    sign_in_as(@admin)
    patch active_ingredient_path(@active_ingredient), params: { active_ingredient: { name: "Updated", half_life: 5.0 } }
    assert_redirected_to active_ingredient_path(@active_ingredient)
    assert_equal "Updated", @active_ingredient.reload.name
  end

  test "should not update active ingredient with invalid params" do
    sign_in_as(@admin)
    patch active_ingredient_path(@active_ingredient), params: { active_ingredient: { name: "" } }
    assert_response :unprocessable_content
  end

  # destroy
  test "should redirect destroy if not authenticated" do
    delete active_ingredient_path(@active_ingredient)
    assert_redirected_to new_session_path
  end
