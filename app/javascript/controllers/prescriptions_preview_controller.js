...
const formatConc = (c, prefix = "") => c < 0.001 ? `${prefix}${c.toExponential(2)}` : `${prefix}${c.toFixed(3)} mg/L`
  ...
  connect() {
    ...
    const concentrationAt = (ing, t) =>
      ing.doses.reduce((sum, dose) => {
        ...
        const ke = Math.log(2) / ing.halfLife, ka = ing.absorptionRate, vd = ing.volumeOfDistribution
        if (Math.abs(ka - ke) < 0.0001) return sum
        return sum + Math.max(0, (dose.amount / vd) * (ka / (ka - ke)) * (Math.exp(-ke * elapsed) - Math.exp(-ka * elapsed)))
      }, 0)
    ...
    const localMaxima = ingredients.flatMap((ing, idx) => {
      const pts = data.filter(d => d.ingredient === ing.name)
      return pts
        .filter((d, i) => i > 0 && i < pts.length - 1 && d.concentration > pts[i - 1].concentration && d.concentration > pts[i + 1].concentration)
    ...
    const chart = Plot.plot({
      ...
      marks: [
        ...
        Plot.text(
          localMaxima.filter(m => m.time !== 0 && (dots.find(d => d.ingredient === m.ingredient)?.concentration ?? 0) < m.concentration * 0.85),
          { x: "time", y: "concentration", text: d => formatConc(d.concentration), dx: 6, dy: -10, fontSize: 10, fontWeight: "500", fill: d => d.color, textAnchor: "start" }
        ),
      ],
    })
