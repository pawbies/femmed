class DiaryEntrySideEffectsController < ApplicationController
  ...
  def create
    side_effect = @diary_entry.diary_entry_side_effects.new diary_entry_side_effect_params
  ...
  private
    ...
    def diary_entry_side_effect_params
      params.expect(diary_entry_side_effect: [ :side_effect_id, :severity, :notes ])
    end
end
